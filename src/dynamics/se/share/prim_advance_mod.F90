...
module prim_advance_mod
  ...
  real (kind=r8) :: initialized_for_dt   = 0
...
contains
  ...
  subroutine prim_advance_exp(elem, deriv, hvcoord, hybrid, dt, tl,  nets, nete)
    ...
    real (kind=r8) ::  eta_dot_dpdn(np,np,nlevp)
  ...
  subroutine applyCAMforcing(elem,hvcoord,np1,np1_qdp,dt_q,nets,nete)
    ...
    integer :: i,j,k,ie,q
    real (kind=r8) :: v1,dp
    real (kind=r8) :: beta(np,np),E0(np,np),ED(np,np),dp0m1(np,np),dpsum(np,np)
    logical :: wet
...
!$omp parallel do private(q,k,i,j,dp)
  ...
  subroutine advance_hypervis_dp(edge3,elem,hvcoord,hybrid,deriv,nt,nets,nete,dt2,eta_ave_w)
    ...
    real (kind=r8) :: dpdn,dpdn0, nu_scale_top
    ...
    real (kind=r8), dimension(np,np,nlev) :: p
    real (kind=r8), dimension(np,np) :: dptemp1,dptemp2
...
! NOTE: PGI compiler bug: when using spheremp, rspheremp and ps as pointers to elem(ie)% members,
  ...
  !       real (kind=r8), dimension(:,:,:), pointer   :: ps
    ...
    real (kind=r8) :: v1,v2,dt,heating,utens_tmp,vtens_tmp,ttens_tmp,dptens_tmp
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if (hypervis_order == 1) then
     if (nu_p>0) call endrun( 'ERROR: hypervis_order == 1 not coded for nu_p>0')
...
!  type (TimeLevel_t)   , intent(in) :: tl
  ...
  ! local
  real (kind=r8) :: eta_ave_w  ! weighting for mean flux terms
  real (kind=r8) :: nu_scale, dpdn,dpdn0, nu_scale_top
  integer :: k,kptr,i,j,ie,ic,nt
  real (kind=r8), dimension(np,np,2,nlev,nets:nete)      :: vtens
...
! NOTE: PGI compiler bug: when using spheremp, rspheremp and ps as pointers to elem(ie)% members,
  ...
  real (kind=r8), dimension(np,np,2) :: lap_v
  real (kind=r8) :: v1,v2,dt,heating,utens_tmp,vtens_tmp,ptens_tmp
...
!$omp parallel do private(k)
  ...
  ! local
  real (kind=r8), pointer, dimension(:,:)      :: ps         ! surface pressure for current tiime level
  real (kind=r8), pointer, dimension(:,:,:)   :: phi

  real (kind=r8), dimension(np,np,nlev)   :: omega_p
  real (kind=r8), dimension(np,np,nlev)   :: T_v
  real (kind=r8), dimension(np,np,nlev)   :: divdp
  real (kind=r8), dimension(np,np,nlev+1)   :: eta_dot_dpdn  ! half level vertical velocity on p-grid
  real (kind=r8), dimension(np,np)      :: sdot_sum   ! temporary field
  real (kind=r8), dimension(np,np,2)    :: vtemp     ! generic gradient storage
  real (kind=r8), dimension(np,np)      :: vgrad_T    ! v.grad(T)
  real (kind=r8), dimension(np,np)      :: Ephi       ! kinetic energy + PHI term
  real (kind=r8), dimension(np,np,2)      :: grad_ps    ! lat-lon coord version
  real (kind=r8), dimension(np,np,2,nlev) :: grad_p
  real (kind=r8), dimension(np,np,nlev)   :: vort       ! vorticity
  real (kind=r8), dimension(np,np,nlev)   :: p          ! pressure
  real (kind=r8), dimension(np,np,nlev)   :: dp         ! delta pressure
  real (kind=r8), dimension(np,np,nlev)   :: rdp        ! inverse of delta pressure
  real (kind=r8), dimension(np,np,nlev)   :: T_vadv     ! temperature vertical advection
  real (kind=r8), dimension(np,np,nlev)   :: vgrad_p    ! v.grad(p)
  real (kind=r8), dimension(np,np,nlev+1) :: ph               ! half level pressures on p-grid
  real (kind=r8), dimension(np,np,2,nlev) :: v_vadv   ! velocity vertical advection
  real (kind=r8) ::  kappa_star(np,np,nlev)
  real (kind=r8) ::  vtens1(np,np,nlev)
  real (kind=r8) ::  vtens2(np,np,nlev)
  real (kind=r8) ::  ttens(np,np,nlev)

  real (kind=r8) ::  cp2,cp_ratio,E,de,Qt,v1,v2
  real (kind=r8) ::  glnps1,glnps2,gpterm
...
!$omp parallel do private(k)
  ...
  integer :: nets,nete
  real (kind=r8), dimension(np,np,nets:nete), intent(inout)   :: phis
  type (hybrid_t)      , intent(in) :: hybrid
  type (element_t)     , intent(inout), target :: elem(:)
  type (derivative_t)  , intent(in) :: deriv
  real (kind=r8), intent(in)   :: minf
  integer,               intent(in) :: numcycle

  ! local
  real (kind=r8), dimension(np,np,nets:nete) :: pstens
  real (kind=r8), dimension(nets:nete) :: pmin,pmax
  real (kind=r8) :: mx,mn
